...
% from descriptor.proto (libprotoc 3.40.0). Eventually, this will be bootstrapped
...
protobufs:protoc_gen_swipl_version(descriptor_proto, [0,9,2]).
...
main(Path) :-
    sanity_check,
    open(Path, read, Fd, [encoding(octet), type(binary)]),
    read_stream_to_codes(Fd, WireCodes),
    close(Fd),
    protobuf_parse_from_codes(WireCodes, '.google.protobuf.FileDescriptorSet', Msg),
    write_metadata(Msg).
...
% (parse_descriptor/1) over the output of
% protoc --decode=FileDescriptorSet /path/to/plugin.proto
% (see Makefile rules %.proto.parse, %.proto.wiredump,
% <...>/plugin.proto.wire).
descriptor_set(
  'FileDescriptorSet'{
